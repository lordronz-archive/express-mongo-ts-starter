...
describe('Error middlewares', () => {
  describe('Error converter', () => {
    ...
    test('should convert an Error to ApiError and preserve its status and message', () => {
      const error: Error & { statusCode?: number } = new Error('Any error');
    ...
    test('should convert a Mongoose error to ApiError with status 400 and preserve its message', () => {
      const error: Error & { statusCode?: number } = new mongoose.Error(
        'Any mongoose error'
      );
      const next = jest.fn();
  ...
  describe('Error handler', () => {
    beforeEach(() => {
      jest.spyOn(logger, 'error').mockImplementation(() => createLogger());
    });

    test('should send proper error response and put the error message in res.locals', () => {
      const error: Error & { statusCode?: number } = new ApiError(
